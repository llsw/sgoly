...
function sgoly_record.opt_update(nickname, win_money, cost_money, win_times, times, 
	...
	if(false == res) then
		...
		if(nil ~= win_money) then
			local sql = string.format("update sgoly.record set win_money = '%d' where"
		...
		if(nil ~= cost_money) then
			local sql = string.format("update sgoly.record set cost_money = '%d' where"
		...
		if(nil ~= win_times) then
			local sql = string.format("update sgoly.record set win_times = '%d' where"
		...
		if(nil ~= times) then
			local sql = string.format("update sgoly.record set times = '%d' where"
		...
		if(nil ~= single_max) then
			local sql = string.format("update sgoly.record set single_max = '%d' where"
		...
		if(nil ~= continuous_max) then
			local sql = string.format("update sgoly.record set continuous_max = '%d' where"
